...
int main(){
    ...
    MemoryBlock **RAM = createMemoryRAM(SIZE_RAM);
    MemoryBlock **cache_01 = createMemoryCache(SIZE_CACHE_01);
    MemoryBlock **cache_02 = createMemoryCache(SIZE_CACHE_02);
    MemoryBlock **cache_03 = createMemoryCache(SIZE_CACHE_03);
    Instructions **instructions;

    instructions = malloc(SIZE_INSTRUCTIONS * sizeof(Instructions*));
    ...
    while (instructions[PC]->opcode != -1){
        ...
        if(instructions[PC]->opcode != -1){
            ...
            case ADDITION:
                    ...
                    cache_01[third_cache_position]->updated = true;
    ...
    custo_total = cache_01[0]->cost + cache_02[0]->cost + cache_03[0]->cost + RAM[0]->cost;
...
MemoryBlock **createMemoryRAM(int size){
    ...
    for (int i = 0; i < size; i++){
        new[i] = createMemoryBlockRAM(size);
        new[i]->address_block = i;
    }
...
MemoryBlock **createMemoryCache(int size){
    ...
    for (int i = 0; i < size; i++){
        new[i] = createMemoryBlockCache(size);
...
int MMU(MemoryBlock **cache_01, MemoryBlock **cache_02, MemoryBlock **cache_03, MemoryBlock **RAM, Address *address){
    ...
    if(searcheCache(cache_01, address->address_block, SIZE_CACHE_01) != -1){
        ...
        cache_position = searcheRAM(RAM, cache_03, address->address_block); 
        cache_position = dataTransfer(cache_03, cache_02, cache_position);
        cache_position = dataTransfer(cache_02, cache_01, cache_position);       
...
int searcheRAM(MemoryBlock **RAM, MemoryBlock **cache_03, int address){
    int emptyPosition = getEmptyPosition(cache_03, SIZE_CACHE_03);

    // Cache Vazia
    if(emptyPosition != -1){
        blockTransfer(cache_03[emptyPosition], RAM[address]);
        ...
        for (int i = 0; i < SIZE_CACHE_03; i++){
            if(cache_03[i]->accessed <= menor){
                menor = cache_03[i]->accessed;
                selectedPosition = i;       
            }
        }
        
        if(cache_03[selectedPosition]->updated == 1){
           
            MemoryBlock* aux = createMemoryBlockCache(1);

            blockTransfer(aux, cache_03[selectedPosition]);
            blockTransfer(cache_03[selectedPosition], RAM[address]);
			blockTransfer(RAM[address], aux);

            blockOutdated(RAM[address]);
            free(aux);
...
int dataTransfer(MemoryBlock **carrierBlock, MemoryBlock **receivingBlock, int cache_position_aux){
    ...
    int emptyPosition = getEmptyPosition(receivingBlock, receivingBlock[0]->size);
    int selectedPosition;
