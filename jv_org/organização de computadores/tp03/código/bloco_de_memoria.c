...
struct bloco{
   int *palavras;
   int endereco_bloco_hd; 
   int endereco_bloco_cache_ram; //Endereço cache e ram
   int atualizacoes;
   time_t tempo; 
};
...
Bloco *Criar_Bloco(){
    ...
    bloco->endereco_bloco_hd = -1;
    bloco->endereco_bloco_cache_ram = -1;
...
void setBloco (Bloco **vetor, Bloco *novo, int indice){
    setEndereco_bloco_cache(novo, indice);
    vetor[indice] = novo;
}
